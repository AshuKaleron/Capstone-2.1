...
with st.sidebar:
    ...
    st.success("✅ Dataset Loaded") if df is not None else st.error("❌ Dataset Missing")
    st.success("✅ AI Model Active") if model is not None else st.error("❌ AI Model Offline")
    st.success("✅ Scaler Ready") if scaler is not None else st.error("❌ Scaler Missing")
    st.success("✅ Metrics Loaded") if champion_info is not None else st.warning("⚠️ No Metrics")
